...
public class NewsIndexer {
	...
	public static final String searchDir = "D:\\学习\\中科院\\2017.9-2018.1第一学期秋季\\信息检索导论\\UCASIR\\UCASIR\\索引文件\\Index";
    ...
    private void createIndex(ResultSet rs) throws Exception {
    	...
    	Logger log=logUtils.createLog(getClass().getName());
    	log.info("正在创建索引 ... ..."); //插入的日志内容
...
//				StringField不分析，TextField分析
        ...
        try {  
            ...
            indexWriter.close();  
            directory.close();  
            log.info("索引创建完成, 总计用时 " + (System.currentTimeMillis()-t0)/1000 + " s"); //插入的日志内容
