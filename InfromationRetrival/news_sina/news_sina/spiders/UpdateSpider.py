...
class NewsSpider(scrapy.Spider):
    ...
    yes = cur + datetime.timedelta(days=-4)
    if(cur.month==yes.month):
        month = [cur.month]
        day = list(range(yes.day,cur.day))
    else:#如果是1、2、3日，只更新当天的新闻
        month=[cur.month]
        day=[cur.day]
    ...
    for i in month:
        for j in day:
            start_urls.append("http://roll.news.sina.com.cn/interface/rollnews_ch_out_interface.php?col=89&date=2018-"+str(i).zfill(2)+"-"+str(j).zfill(2)+"&ch=01&num=60&page=1")
